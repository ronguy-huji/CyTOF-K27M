...
def wfall(shap_values, max_display=10, show=True):
    ...
    """
    dark_o= mpl.colors.to_rgb('darkorange')
    dim_g= mpl.colors.to_rgb('dimgray')
...
color01= mpl.colors.to_rgb('darkorange')
color04= mpl.colors.to_rgb('dimgray')
...
def ABS_SHAP(df_shap,df):
    ...
    corr_df['Sign'] = np.where(corr_df['Corr']>0,'dimgray','darkorange')
    ...
    colorlist = k2['Sign']
#    print(colorlist)
    ax = k2.plot.barh(x='Variable',y='SV',color = colorlist, figsize=(5,6),legend=False)
...
sns.pairplot(X[NMS+['clust']],hue='clust',corner=True,
              diag_kind="kde",diag_kws=dict(fill=True, common_norm=False),
              palette={0:'darkorange',1:'dimgray'})