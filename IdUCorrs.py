...
# -*- coding: utf-8 -*-
"""
Created on Thu May 13 15:39:01 2021
...
for NN in NamesCorr:
    plt.figure(figsize=(10,10))
    sns.kdeplot(df[mask][NN])
    plt.title(ExpName)
    plt.show()
    plt.figure(figsize=(10,10))
    sns.kdeplot(data=df[mask],x=NN,y='IdU',levels=20,fill=True)
    plt.title(ExpName)
    plt.show()
    plt.figure(figsize=(10,10))
    sns.scatterplot(data=df[mask],x=NN,y='IdU',s=4)
    plt.title(ExpName)
    plt.show()