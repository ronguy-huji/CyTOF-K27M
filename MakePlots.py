...
aaa=pd.read_csv(dir+"C06-C08_Norm.csv")
...
sns.pairplot(EqSize[CompVars].sort_values("Type",ascending=False),
             corner=True,hue="Type",plot_kws={"s": 3},palette=['blue','red']
        )
plt.title(title)
...
CorrDiff=dfmut[Names_All].corr()-df[Names_All].corr()
...
plt.title(F2+"-"+F1+" Correlations"); 
plt.savefig("Plots/"+F1+"_"+F2+"_CorrDiff.png",dpi=100,bbox_inches = 'tight')
plt.show()