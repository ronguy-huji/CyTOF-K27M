...
C11=pd.read_csv(dir+"C11_Norm_UMAP.csv")
C12=pd.read_csv(dir+"C12_Norm_UMAP.csv")
...
for NN in Names:
    ...
    cc=C12[TSNEVar]#[mask]
    plt.figure(figsize=(6, 5))
    plt.scatter(C12['umap0'],C12['umap1'],s=2,
                c=cc, cmap=plt.cm.jet)
    plt.colorbar()
    plt.clim(-3.5,3.5)
...
for i in range(0,2):
    mask=C12['clust']==i    
    plt.scatter(C12[mask]['umap0'],C12[mask]['umap1'],s=2,
                c=cList[i],label="Cluster "+str(i))    
plt.legend(markerscale=5);
...
#sns.pairplot(C11[NMS],hue='clust',palette="tab10",corner=True)

plt.figure(figsize=(6, 5))
plt.scatter(C11['H3K27M'],C12['CXCR4'],s=2,
            c=cc, cmap=plt.cm.jet)
plt.colorbar()
plt.clim(-3.5,3.5)
plt.title(TSNEVar+" C12")