...
C16=pd.read_csv(dir+"C05.csv")
C17=pd.read_csv(dir+"C07.csv")
C18=pd.read_csv(dir+"C15C16/C15.csv")
...
C16=C16[(C16[GateColumns1]>0).all(axis=1)]
C17=C17[(C17[GateColumns2]>0).all(axis=1)]
C16=C16[GateColumns1]
C17=C17[GateColumns2]

C18=C18[(C18[GateColumns2]>0).all(axis=1)]
C18=C18[GateColumns2]
...
for count, value in enumerate(dd0.index):
    ...
    ax.scatter(y=0, x=dd1[value], s=50, color=colorVal, alpha=0.7,label=value)
    ax.scatter(y=1, x=dd2[value], s=50, color=colorVal, alpha=0.7)
    ax.plot([dd1[value],dd2[value]],[0,1],color=colorVal,linewidth=2)
    if value in C16.columns:
        ax.scatter(y=2, x=dd0[value], s=50, color=colorVal, alpha=0.7)
        ax.plot([dd2[value],dd0[value]],[1,2],color=colorVal,linewidth=2)
ax.set_xlim(-1.2, 3)
plt.yticks([0,1,2], ['C07','C15','C05'], rotation='horizontal')    
plt.legend()
plt.title('$log(\sigma/\mu)$ - HEK Mutant')
