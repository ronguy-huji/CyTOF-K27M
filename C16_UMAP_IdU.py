...
df=CAll.copy()
df=df[df.IdUHigh==True]

q1 = df['IdU'].quantile(0.25)

mask = df['IdU'].between(df['IdU'].min(), q1, inclusive=True)
mask2 = df['clust']<3
ExpName="C16 Q1 IdU High"

for NN in NamesCorr:
    plt.figure(figsize=(10,10))
    sns.kdeplot(data=df[mask & mask2],x=NN,hue='clust',palette={'red','blue'},common_norm=(False))
    plt.title(ExpName)
    plt.show()
    plt.figure(figsize=(10,10))
    sns.kdeplot(data=df[mask & mask2],x=NN,y='IdU',levels=20,fill=False,
                 hue='clust',palette={'red','blue'},common_norm=(False))
    plt.title(ExpName)
    # plt.show()
    # plt.figure(figsize=(10,10))
    # sns.scatterplot(data=df[mask & mask2],x=NN,y='IdU',s=4)
    # plt.title(ExpName)
    # plt.show()
