...
C11=pd.read_csv(dir+"C11_No_Norm.csv")
C12=pd.read_csv(dir+"C12_No_Norm.csv")
...
for NN in Names:
    ...
    plt.colorbar()
#    plt.clim(0,10)
    plt.title(TSNEVar+" C11 No Normalization")
...
for NN in Names:
    ...
    plt.colorbar()
#    plt.clim(0,10)
    plt.title(TSNEVar+" C12 No Normalization")
...
ax.hlines(y=dd0.index, xmin=-10, xmax=10, color='gray', alpha=0.7, 
...
ax.set_title('C11 Mean of Distributions No Norm', fontdict={'size':22})
ax.set_xlim(0,10)
...
ax.set_title('C12 Mean of Distributions No Norm', fontdict={'size':22})
ax.set_xlim(0,10)
...
g.fig.suptitle("C11 No Norm", y=1.08) # y= some height>1


g=sns.pairplot(C12[C12['clust']!=-1][NMS],hue='clust',palette="tab10",corner=True,
               diag_kind="kde",diag_kws=dict(fill=True, common_norm=False),)
g.fig.suptitle("C12 No Norm", y=1.08) # y= some height>1