...
dir="/Users/ronguy/Dropbox/Work/CyTOF/Datasets/C15C16/"
C15=pd.read_csv(dir+"C15.csv")
C16=pd.read_csv(dir+"C16_JustInduced.csv")
#C16=pd.read_csv(dir+"C16.csv")


renamelist={'H2AK119ub' : 'H2Aub','pHistone H3 [S28]':'pH3[S28]','pHistone H2A.X [Ser139]':'yH2A.X',
            'H4K16Ac':'H4K16ac'}
C15.rename(columns=renamelist,inplace=True)
C16.rename(columns=renamelist,inplace=True)
...
for col in C15.columns:
    lG=  np.count_nonzero(C15[col]>1)  
    print("%-30s %d %5.3f" % (col,lG,lG/lC15*100.)) 
print(" ")
lC16=len(C16)
print("C16")
for col in C16.columns:
    lG=  np.count_nonzero(C16[col]>1)  
...
C15=C15[(C15[GateColumns]>5).all(axis=1)]
...
C15mask27M=C15['H3K27M']<1
C16mask27M=C16['H3K27M']<1
#C15maskpH3=C15['pHistone H3 [S28]']==0
...
dfmask=C15mask.append(C16mask)
#dfmask=C16mask

C16Bck=C16.copy()

scFac=5
C15=np.arcsinh(C15/scFac)
...
CAllN=CAll.copy()
CAllN=CAllN.assign(umap0=X_2d[:,0])
CAllN=CAllN.assign(umap1=X_2d[:,1])


CAllRed=CAllN[CAllN.type=="Mutant"].sample(n=round(len(C15)/1)).copy()
CAllRed=CAllRed.append(CAllN[CAllN.type=="WT"])
...
#                 color=rgba) 
#     plt.title(TSNEVar+" C15/C16")
#     plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/UMAP_"+TSNEVar+"_C15C16.pdf",
...
for NN in NamesAll:
    ...
    plt.scatter(X_2d[mask][:,0],X_2d[mask][:,1],s=2,
                color=rgba) 
    plt.title(TSNEVar+" C15/C16")
#    plt.title(TSNEVar+" C16")
    plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/UMAP_"+TSNEVar+"_C15C16.png",
#    plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/UMAP_"+TSNEVar+"_C16.png",
                dpi=100,bbox_inches = 'tight')
    plt.show()
...
d0=C15.copy()#CAll[CAll['type']=='WT' ]
d1=C16.copy()#CAll[CAll['type']=='Mutant' ]

#d0[C15mask]=float("nan")
...
plt.title('C16 C15 Diffs', fontdict={'size':20})
plt.grid(linestyle='--', alpha=0.5)
plt.show()

### Box Plots 
CAll=C15.assign(Type='WT')
CAll=CAll.append(C16.assign(Type='Mutant'))
for NN in DispList:
     BoxVar=NN
     plt.figure(figsize=(6, 5))    
...
plt.title('C15 Closed Chromatin',y=1.5)
...
plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)
plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/C15_ClosedCh.pdf")
plt.title('C15 Closed Chromatin')
plt.show()
