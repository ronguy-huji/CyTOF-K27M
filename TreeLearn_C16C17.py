...
df=pd.read_csv(dir+"C16_C17_Norm_Clust_Full_Panel.csv")
dfmut=df
...
clf = tree.DecisionTreeClassifier(random_state=0, max_depth=5, min_samples_leaf=50)
...
plt.figure(figsize=(6, 5))    
plt.scatter(X_train['tsne0'],X_train['tsne1'],c=y_train,s=2,cmap=plt.cm.jet)
plt.title('Trained')

plt.figure(figsize=(6, 5))    
plt.scatter(X_test['tsne0'],X_test['tsne1'],c=pred,s=2,cmap=plt.cm.jet)
plt.title('Predicted')
...
FeatureDict=(dict(zip(X.columns, clf.feature_importances_)))
...
for i in range(0,6):
    mask=df['clust']==i
    plt.scatter(df[mask]['tsne0'],df[mask]['tsne1'],s=2,color=clist[i],label="Cluster "+str(i))
plt.legend(markerscale=5.)  
plt.title("C16/C17 Full Panel Clusters")
plt.show()