...
dir="/Users/ronguy/Dropbox/Work/CyTOF/Datasets/C05C07/"
C05=pd.read_csv(dir+"C05.csv")
C07=pd.read_csv(dir+"C07_JustInduced.csv")


renamelist={'H2AK119ub' : 'H2Aub','pHistone H3 [S28]':'pH3[S28]','pHistone H2A.X [Ser139]':'yH2A.X',
            'H4K16Ac':'H4K16ac'}
C05.rename(columns=renamelist,inplace=True)
C07.rename(columns=renamelist,inplace=True)
...
lC05=len(C05)
print("C05")
for col in C05.columns:
    lG=  np.count_nonzero(C05[col]>1)  
    print("%-30s %d %5.3f" % (col,lG,lG/lC05*100.)) 
print(" ")
lC07=len(C07)
print("C07")
for col in C07.columns:
    lG=  np.count_nonzero(C07[col]>1)  
    print("%-30s %d %5.3f" % (col,lG,lG/lC07*100.)) 
...
C05=C05[(C05[GateColumns]>5).all(axis=1)]
C07=C07[(C07[GateColumns]>5).all(axis=1)]
...
C05=C05[(C05[GateColumns]>0).all(axis=1)]
C07=C07[(C07[GateColumns]>0).all(axis=1)]

C05mask27M=C05['H3K27M']<1
C07mask27M=C07['H3K27M']<1
#C05maskpH3=C05['pHistone H3 [S28]']==0
#C07maskpH3=C07['pHistone H3 [S28]']==0

C05mask=C05<2
C07mask=C07<2


dfmask=C05mask.append(C07mask)
...
#C05.loc[C05mask27M,'H3K27M']=-100
#C07.loc[C07mask27M,'H3K27M']=-100
#C05.loc[C05maskpH3,'pHistone H3 [S28]']=-100
#C07.loc[C07maskpH3,'pHistone H3 [S28]']=-100

C05=C05.assign(type='WT')
C07=C07.assign(type='Mutant')
...
#                 color=rgba) 
#     plt.title(TSNEVar+" C05/C07")
#     plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/UMAP_"+TSNEVar+"_C05C07.pdf",
...
for NN in NamesAll:
    ...
    plt.scatter(X_2d[mask][:,0],X_2d[mask][:,1],s=2,
                color=rgba) 
    plt.title(TSNEVar+" C05/C07")
    plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/UMAP_"+TSNEVar+"_C05C07.png",
                dpi=100,bbox_inches = 'tight')
    plt.show()
...
d0=C05.copy()#CAll[CAll['type']=='WT' ]
d1=C07.copy()#CAll[CAll['type']=='Mutant' ]

#d0[C05mask]=float("nan")
#d1[C07mask]=float("nan")
...
plt.title('C07 C05 Diffs', fontdict={'size':20})
plt.grid(linestyle='--', alpha=0.5)
plt.show()

### Box Plots 
CAll=C05.assign(Type='WT')
CAll=CAll.append(C07.assign(Type='Mutant'))
for NN in DispList:
     BoxVar=NN
     plt.figure(figsize=(6, 5))    
     ax = sns.boxplot(x="Type", y=NN, data=CAll,showfliers=False,palette=['blue','red'])
     #plt.title(NN+" C07 Cell Cycle")
     plt.show()
...
plt.yticks(rotation=0); 
plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)
plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/C05_OpenCh.pdf")
plt.title('C05 Open Chromatin')
plt.show()
...
plt.title('C05 Closed Chromatin',y=1.5)
...
plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)
plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/C05_ClosedCh.pdf")
plt.title('C05 Closed Chromatin')
plt.show()
...
plt.yticks(rotation=0); 
plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)
plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/C07_OpenCh.pdf")
plt.title('C07 Open Chromatin')
plt.show()
...
plt.yticks(rotation=00); 
plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)
plt.savefig("/Users/ronguy/Dropbox/Work/CyTOF/Plots/C07_ClosedCh.pdf")
plt.title('C07 Closed Chromatin')
plt.show()
